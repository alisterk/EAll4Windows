...
namespace MiWindows
{
    ...
    public class MiDevice
    {
        ...
        private const int BT_INPUT_REPORT_LENGTH = 21;
        private HidDevice hDevice;
        private string Mac;
        private MiState cState = new MiState();
        private MiState pState = new MiState();
        ...
        private void performDs4Input()
        {
            ...
            while (true)
            {
                ...
                //cState.DpadUp = ((byte)inputReport[4] & Convert.ToByte(7)) != 0 ||
                //                ((byte)inputReport[4] & Convert.ToByte(1)) != 0 || inputReport[4] == Convert.ToByte(0);
                //cState.DpadRight = ((byte)inputReport[4] & Convert.ToByte(1)) != 0 ||
                //                ((byte)inputReport[4] & Convert.ToByte(2)) != 0 || ((byte)inputReport[4] & Convert.ToByte(3)) != 0;
                //cState.DpadDown = ((byte)inputReport[4] & Convert.ToByte(3)) != 0 ||
                //                ((byte)inputReport[4] & Convert.ToByte(4)) != 0 || ((byte)inputReport[4] & Convert.ToByte(5)) != 0;
                //cState.DpadLeft = ((byte)inputReport[4] & Convert.ToByte(5)) != 0 ||
                //                ((byte)inputReport[4] & Convert.ToByte(6)) != 0 || ((byte)inputReport[4] & Convert.ToByte(7)) != 0;
        ...
        public MiState getCurrentState()
        {
            return cState.Clone();
        }

        public MiState getPreviousState()
        {
            return pState.Clone();
        }

        public void getExposedState(MiStateExposed expState, MiState state)
        {
            cState.CopyTo(state);
        ...
        public void getCurrentState(MiState state)
        {
            cState.CopyTo(state);
        }

        public void getPreviousState(MiState state)
        {
            pState.CopyTo(state);
        }
        ...
        {
            if (cState.Y || cState.X || cState.B || cState.A)
                return false;
            if (cState.DpadUp || cState.DpadLeft || cState.DpadDown || cState.DpadRight)
                return false;
            if (cState.LS || cState.RS || cState.L1 || cState.R1 || cState.Back || cState.Menu)
                return false;
